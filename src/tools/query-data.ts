...
interface QueryEnv {
	BGEE_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
	const handler = createQueryDataHandler("BGEE_DATA_DO", "bgee");

	server.registerTool(
		"bgee_query_data",
		{
			title: "Query Staged Bgee SPARQL Results",
			description:
				"Query staged Bgee SPARQL results using SQL. Use this when SPARQL responses are too large and have been staged with a data_access_id.",
			inputSchema: {
